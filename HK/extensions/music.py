...
class Track:
    ...
    def thread(func):
        ...
        def wrapper(*args, **kwargs) -> asyncio.Future:
            return args[0].ctx.bot.loop.run_in_executor(
...
class Queue(asyncio.Queue):
    ...
    def embed(self, track):
        p = track.pafy
        e = self.ctx.bot.embed(
            title="Now Playing", description=p.title, color=self.ctx.bot.color
        )
        ...
        e.set_author(
            name=str(track.ctx.author.name), icon_url=str(track.ctx.author.avatar)
        )
        return e
    ...
    async def play(self, track):
        ...
        self.ctx.voice_client.play(
            stream, after=lambda _: self.ctx.bot.loop.create_task(self.next())
        )
        track.source = stream
        self.playing = track
...
class Music(commands.Cog):
    ...
    async def play(self, ctx, *, query: str) -> None:
        async with ctx.typing():
            ...
            if re.match(
                r"^(?!.*\?.*\bv=)https:\/\/www\.youtube\.com\/.*\?.*\blist=.*$", query
            ):
                tracks = await Track.get_playlist_tracks(ctx, query)
                embed = ctx.bot.embed(
                    title=f"Queued {len(tracks)} items!", color=ctx.bot.color
                )
                embed.clear_fields()
    ...
    def get_queue(self, ctx, *, error=False):
        res = (
            queue if ctx.voice_client and (queue := self.queues[ctx.guild.id]) else None
        )
    ...
    async def goto(self, ctx, seconds: int):
        ...
        if (queue := self.get_queue(ctx, error=True)) and queue.playing:
            ...
            source = FFmpegPCMAudio(
                await queue.playing.audio(),
                before_options=f"-ss {seconds}s " + FFMPEG_OPTIONS["before_options"],
                options=FFMPEG_OPTIONS["options"],
            )
            stream = CustomAudio(source, volume=0.5)
            ctx.voice_client.play(
                stream, after=lambda _: self.bot.loop.create_task(queue.next())
            )
    ...
    async def queue(self, ctx):
        if queue := self.get_queue(ctx, error=True):
            ...
            for index in range(0, len(items), 10):
                ...
                songs = [
                    f"{i+1}. {shorten(track.pafy.title, 55)} [{track.pafy.length}s] ~ {track.ctx.author.mention}"
                    for i, track in enumerate(tracks)
                ]
                desc = "\n".join(songs)
                embed = ctx.bot.embed(
                    title="Queue", description=desc, color=self.bot.color
                )
            ...
            if units:
                ...
                units.append(
                    Unit(
                        embed=ctx.bot.embed(
                            title="Queue", color=self.bot.color, description=desc
                        )
                    )
                )
