...
'''
Simple Music Bot made with Pafy and FFMPEG by Zeffo#9673
'''
...
class Music(commands.Cog):
    """ Crystal clear audio, straight from YouTube to your ears. """
    def __init__(self, bot):
        self.bot = bot
        self.queues = {} # Store a Queue for each guild
        self.bot.loop.create_task(self._self_deafen())
        self.session = aiohttp.ClientSession()
        self.yt_key = bot.yt_key
        pafy.set_api_key(bot.yt_key)
    ...
    def embed(self, p):
        e = discord.Embed(title=p.title, color=self.bot.color)
    ...
    @commands.command(usage='`hk play (song name | YouTube URL)`')
    async def play(self, ctx, *, song: str):
        '''A command that searches and plays audio from YouTube.'''
        if not ctx.author.voice:
            return await ctx.send('You must be in a Voice Channel!', delete_after=5)
        elif ctx.guild.voice_client:
            q = self.queues.get(ctx.guild.id)
            item = await self._parse(song, ctx)
            if item:
                q.put(item)
                await ctx.send(content='`Added to the queue!`', embed=self.embed(self.queues.get(ctx.guild.id).last()))
            else:
    ...
    @commands.command(usage='`hk volume (0 to 1)`')
    ...
    @commands.command(usage='`hk stop`')
    ...
    async def _parse(self, _input, ctx):
        ...
        if 'watch?' in _input:
            ...
            p = await self.run_async(pafy.get_playlist2, _input)
            items =  [item for item in p]
            for i in items:
                i.req = ctx.author
            return items
        else:
    ...
    async def _handler(self, ctx):
        ...
        if queue.empty() and not queue.loop:
            ...
            await ctx.send(content='`Now playing:`', embed=self.embed(queue.np))
        elif queue.loop:
    ...
    @commands.command(usage='`hk queue`')
    async def queue(self, ctx):
        '''A command that lists the music queue. '''
        if ctx.guild.id in self.queues or len(self.queues.get(ctx.guild.id, [])) > 0:
            embed = self.queue_embed(self.queues[ctx.guild.id])
        else:
    ...
    @commands.command(usage='`hk remove (index)`')
    async def remove(self, ctx, element: int=0):
    ...
    @commands.command(usage='`hk skip`')
    async def skip(self, ctx):
        ...
        await ctx.send('Skipping immediately!')
    ...
    @commands.command(usage='`hk pause`')
    ...
    @commands.command(usage='`hk resume`')
    ...
    @commands.command(aliases=['np'], usage='`hk nowplaying/np`')
    async def nowplaying(self, ctx):
        '''A command that displays the song which is currently playing. '''
        q = self.queues.get(ctx.guild.id)
        if q and q.np:
            await ctx.send(embed=self.embed(q.np))
        else:
            await ctx.send('`No songs are currently playing! Use ♫play (song) to play a song from YouTube in your voice channel.``')


    @commands.command(usage='`hk loop`')
    async def loop(self, ctx):
        ...
        if queue := self.queues.get(ctx.guild.id):
            if not queue.loop:
                ...
                await ctx.send(content='`Looping!`', embed=self.embed(queue.np))
            else:
                queue.loop = []
                await ctx.send('`Stopping loop!`')

    
    @commands.command(usage='`hk lq`')
    async def lq(self, ctx):
        '''I have no clue what this is supposed to do. I probably wrote this for debugging and testing purposes and forgot to delete it. Use at your own risk! (and mine actually)'''
        if queue := self.queues.get(ctx.guild.id):
            await ctx.send(', '.join([i.title for i in queue.loop]))
            await ctx.send(queue.loop_index)
